...
//#define Timed(x) Timeout = 0x0FFF; while (x) { if (Timeout-- == 0){ }}

void stm_I2C_Init(I2C_TypeDef * I2Cx, u32 I2Cspeed);
int stm_I2C_StartAndAddr(uint8_t address8bit, uint8_t direction);
...
void stm_I2C_Init(I2C_TypeDef * I2Cx, u32 I2Cspeed){//, u8 SlaveAddr8) {
...
void stm_I2C_Init(I2C_TypeDef * I2Cx,u32 I2Cspeed) {//, u8 SlaveAddr8) {
    ...
    GPIO_PinRemapConfig(GPIO_Remap_I2C1, DISABLE); //for PB6/7
...
int stm_I2C_StartAndAddr(uint8_t slave_addr8, uint8_t direction) { //slave_addr = 8bit addr
	uint32_t Timeout = 0;
	int ret = 0;
	...
	Timed(!I2C_CheckEvent(gI2Cx, I2C_EVENT_MASTER_MODE_SELECT)); //EV5. After sending the START condition the master has to wait for this event. It means that the Start condition has been correctly released on the I2C bus (the bus is free, no other devices is communicating).
	...
	if (direction == I2C_Direction_Transmitter) {
		Timed(!I2C_CheckEvent(gI2Cx, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)); //On rcving ACK from Slave.
...
void stmI2cModuleConfig(I2C_TypeDef* I2Cx, u32 I2Cspeed){
