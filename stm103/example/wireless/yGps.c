...
extern void stmOzOled_showClock(unsigned h, unsigned m, unsigned s, unsigned ypos);
#endif
...
char stmGpsDisplayTimeOfDay(unsigned char uartId, struct minmea_sentence_rmc *rmc)
{
    ...
    if(rmc != NULL){
       		...
       		stmOzOled_showClock(rmc->time.hours, rmc->time.minutes, rmc->time.seconds, ypos);
			stmOzOLED_printString("GPSOK",1,ypos+4,15);
		#endif
	}else{
		#if (DISPLAY_ON == DISPLAY_ON_MAX7219_SPI)
			...
			stmOzOLED_printString("NoGPS",1,ypos+4,15);
		#endif
	}
    return ERR_OK;
}
...
void stmGpsLoop(unsigned char uartId)
{
	...
	while(1) //main loop
    {
		...
		if(len > 0){
			...
			stmOzOLED_printString("GPS RX...",1,3,15); //Print the String
		}
		delayms(100);
