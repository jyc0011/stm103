...
rep:	if(mode){
		...
		//Tea_SearchUp(tea5767.Tea_Frequency);
	}else{
		tea5767.Tea_Frequency -= 100;
		if(tea5767.Tea_Frequency < minFreq)
			tea5767.Tea_Frequency = maxFreq;
		//Tea_SearchDown(tea5767.Tea_Frequency);
	}
...
void Tea_Tuning(unsigned char nextb, unsigned char prevb){
	...
	FreqL = FreqL/100;
	if(stereo)
		//sprintf(iastr,"%dMHz:Lv=%d:St",tea5767.Tea_Frequency,level);
