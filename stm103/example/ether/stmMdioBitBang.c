...
#elif ((PROCESSOR == PROCESSOR_STM32F103C8T6) || (PROCESSOR == PROCESSOR_STM32F107VCT6))
#include <string.h>
#include <stdarg.h>
#include "stm32f10x.h"
...
static void stmOutputBitStreamMdio(unsigned value, unsigned n)
{
	...
	if(g_mdio_pinmode !=  MDIO_PIN_IS_OUTPUT_MODE)
	{
		//MDIO-PB4 -- Set Output
		GPIO_InitStruct.GPIO_Pin = GPIO_Pin_4;//PB4
		GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_OD;//GPIO_Mode_Out_PP; //GPIO_Mode_OUT; 		// we want the pins to be an output
		GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz; 	// this sets the GPIO modules clock speed
		GPIO_Init(GPIOB, &GPIO_InitStruct);//
		g_mdio_pinmode =  MDIO_PIN_IS_OUTPUT_MODE;
	}
	...
	{	// Output MDIO first
		if (val & 0x80000000)
			GPIO_SetBits(GPIOB, GPIO_Pin_4); // PB4 Set high
		else
			GPIO_ResetBits(GPIOB, GPIO_Pin_4); // PB4 Set high		// Set low
		//after some delay, issue rising MDC pulse
		somedelay(g_delayval);
		GPIO_SetBits(GPIOB, GPIO_Pin_3);	// Set High MDC -- Peer will latch on this rising edge.
		//after some delay, issue falling MDC pulse
		somedelay(g_delayval);
		GPIO_ResetBits(GPIOB, GPIO_Pin_3);   // Set Low MDC
	}
	somedelay(g_delayval>>4); //at last stay low level.
}
...
static void stmTurnaround_MDIO(void)
{
	...
	somedelay(g_delayval);
	GPIO_ResetBits(GPIOB, GPIO_Pin_3);   // Set Low MDC
	GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_4); //Dummpy.
	somedelay(g_delayval);
}
...
void stmMdioBitbangPortCheck(void){
	while(1){
		GPIO_SetBits(GPIOB, GPIO_Pin_4); // PB3-MDC-Low
		GPIO_SetBits(GPIOB, GPIO_Pin_3); // PB3-MDC-Low
		delayms(100);
		GPIO_ResetBits(GPIOB, GPIO_Pin_4); // PB3-MDC-Low
		GPIO_ResetBits(GPIOB, GPIO_Pin_3); // PB3-MDC-Low
		delayms(100);
	}
}
