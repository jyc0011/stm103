 ...
 *                            +------------+
 *  [CONTROL INTERFACE]
 *  MDIO for RTL9K = 0x01; IP101A=0x02
...
void stmMdioInitPins(void)
{
	...
	GPIO_InitS.GPIO_Pin = GPIO_Pin_14 | GPIO_Pin_15;
	GPIO_InitS.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitS.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitS);

	GPIO_ResetBits(GPIOC, GPIO_Pin_14); //MDC LOW
...
u32 stmMdio_GetBasicRegister(u8 MMDid,u16 reg){
	...
	if(PhyMdioModule.clause45 == 0x45){
		...
		if(PhyMdioModule.platform == PHYIS_RTL9047SWITCH){
			retval = mdio_read(PhyMdioModule.phyaddr, reg); //read reg
		}else{
			retval = mdio_read(PhyMdioModule.phyaddr, reg); //read reg
		}
	}
...
u16 yKSZ9021_GetExtendedReg(u16 reg){
	...
	retval = mdio_read(PhyMdioModule.phyaddr, 0x0d);
}
...
u32 RTL9047AASwitch_GetNormalRegister(u32 reg){
	...
	//retL32 = (u32)mdio_read(0x18, 4); //??
	//retH32 = (u32)mdio_read(0x18, 6); //??

//	delayms(10);
	timeout = 0;
	while(1){
			regData16 = mdio_read(0x18, 6);
	...
	retL32 = 0x0000;
	retH32 = 0x0000;
	retL32 = (u32) mdio_read(0x18, 0); //read LSB
	retH32 = (u32) mdio_read(0x18, 2); //read MSB
...
u32 RTL9047AASwitch_GetSpecialRegister(u16 regaddr){
	...
	retval = mdio_read(0x01, 28); //read reg
	return retval;
}
...
void yRTL8201_StatusHandler(u8 extended){
	...
	if(extended){
		RTL9K_PhyStatus.regf_extstatus = (u16)mdio_read(0x19, 0x0f); //Extended LRE Status
		...
		RTL9K_PhyStatus.reg11_phyextstatus = (u16)mdio_read(0x19, 0x11); //PHY Extended Status
		...
		RTL9K_PhyStatus.reg1a_intstatus = (u16)mdio_read(0x19, 0x1a); //Interrupt Status
	...
	delayms(10);
	RTL9K_PhyStatus.reg1c08_ledstatus = (u16)mdio_read(0x19, 0x1c);
