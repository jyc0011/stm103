		...
		unsigned short freq)
{
	...
	if(channel == CH1){
		word = 0x8000 | 0x0000 | freq;
		Sn76489_WriteShort(word);
	}
	else if(channel == CH2){
		word = 0x8000 | 0x2000 | freq;
		Sn76489_WriteShort(word);
	}
	else if(channel == CH3){
		word = 0x8000 | 0x4000 | freq;
		Sn76489_WriteShort(word);
	}
	else{
		//Not happend
...
void stmSn76489Loop(void)
{
	...
	freq = 100;
    atten = 0x0; //0xf=MUTE
	printf("\r\nSn76489 ToneGen with MCP23S17 Test CH=%u, freq=%u \r\n", channel, freq);
    stmMCP23S17_InitForSn76489(USE_SPI2);

    Sn76489SetToneAttenuation(channel, atten);
    Sn76489SetToneAttenuation(CH2, 0xf); //Mute CH2
    ...
    Sn76489SetToneFreq(CH1, nfreq);
    Sn76489SetToneFreq(CH2, nfreq);
    Sn76489SetToneFreq(CH3, nfreq);
	while(1){
		nfreq = Sn76489GetFreqDivideN(freq);
		Sn76489SetToneFreq(channel, nfreq);
		delayms(3000);
		freq +=100;
		if(freq>1023)
			freq = 100;
